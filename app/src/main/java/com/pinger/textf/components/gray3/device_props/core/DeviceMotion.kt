...
object DeviceMotion {
                ...
                try {
                    ...
                    when (event.sensor.type) {
                        Sensor.TYPE_ACCELEROMETER -> {
                            ...
                            gravity[0] =
                                GRAVITY_FILTER_ALPHA * gravity[0] + (1 - GRAVITY_FILTER_ALPHA) * v[0]
                            gravity[1] =
                                GRAVITY_FILTER_ALPHA * gravity[1] + (1 - GRAVITY_FILTER_ALPHA) * v[1]
                            gravity[2] =
                                GRAVITY_FILTER_ALPHA * gravity[2] + (1 - GRAVITY_FILTER_ALPHA) * v[2]
                        ...
                        Sensor.TYPE_MAGNETIC_FIELD -> lastMagMagnitude =
    ...
    private fun calculateMotionScore(samples: List<Float>): Float {
        val diffs =
            FloatArray(samples.size - 1) { i -> abs(samples[i + 1] - samples[i]) }.apply { sort() }
